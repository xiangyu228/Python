...
"""
  ...
  @ 平 台:请配合 pyspider 使用
  @ 作 用:抓取38dy.cn的部分电影信息保存到者数据库(类型、名称、封面图、下载地址)
...
class Handler(BaseHandler):
    ...
    def film_save_info(self,response):
        ...
        data = [film_type_name,film_title,film_img_url,"",file_info,str(file_download_list),film_add_time]
        try:
            cursor.execute("INSERT INTO yf_films(film_type_name,film_title,film_img_url,film_img_path,film_body,film_download,film_add_time) VALUES (%s,%s,%s,%s,%s,%s,%s)",data)
            connect.commit()
        except:
            print("提交失败",connect.Error)
            connect.rollback()
